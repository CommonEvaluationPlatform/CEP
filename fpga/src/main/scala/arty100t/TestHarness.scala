...
class Arty100TFPGASimTestHarness(override implicit val p: Parameters) extends Arty100TShellBasicOverlays {
  // Complains if not defined here, because not in the fpga-shells abstract BasicOverlays class for Arty (unlike VCU118)
  val pllReset = InModuleBody { Wire(Bool()) }
...
// DOC include end: UartOverlay
  ...
  // module implementation
  override lazy val module = new Arty100TFPGASimTestHarnessImp(this)
}

class Arty100TFPGASimTestHarnessImp(_outer: Arty100TFPGASimTestHarness) extends LazyRawModuleImp(_outer) with HasHarnessSignalReferences {

...
//#************************************************************************
//# CEP VC707 Test Harness
//#************************************************************************
class VC707CEPFPGATestHarness(override implicit val p: Parameters) extends VC707Shell { outer =>
  ...
  // Modify the last field of `DDRDesignInput` for 1GB RAM size
  val ddrNode = dp(DDROverlayKey).head.place(DDRDesignInput(dp(ExtTLMem).get.master.base, dutWrangler.node, harnessSysPLL, false)).overlayOutput.ddr
  val ddrClient = TLClientNode(Seq(TLMasterPortParameters.v1(Seq(TLMasterParameters.v1(
    name = "chip_ddr",
    sourceId = IdRange(0, 1 << dp(ExtTLMem).get.master.idBits)
  )))))
  ...
  // module implementation
  override lazy val module = new VC707CEPFPGATestHarnessImp(this)
}

class VC707CEPFPGATestHarnessImp(_outer: VC707CEPFPGATestHarness) extends LazyRawModuleImp(_outer) with HasHarnessInstantiators {
  override def provideImplicitClockToLazyChildren = true
  val vc707Outer = _outer

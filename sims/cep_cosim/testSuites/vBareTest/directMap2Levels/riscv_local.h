...
//
#define NO_PMP
#define UNUSED_PPN_BITS (0x1ff<<12)

#define CLR_UNUSED_PPN do { \
  LEAF_ENTRY_PTR = (((DRAM_BASE+(level1*MEGAPAGE_SIZE)) >> PGSHIFT) << PTE_PPN_SHIFT) | PTE_V | PTE_R | PTE_W | PTE_X | PTE_U; \
  } while (0)

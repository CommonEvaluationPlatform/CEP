...
void handle_fault_giga(uintptr_t addr, uintptr_t cause)
{
    ...
    //
    if (vpn2pt[level2]) { // assuming they are all unique!!!
        // l1+page_adr = 0x1FF will cause bad virtual address !!!
        if (level2 != 0) {
            vpn2pt[level2] &= ~(0x3FFFF << PTE_PPN_SHIFT); // zero out l1+page_address
        }
        //
        if (!(vpn2pt[level2] & PTE_A)) {
            //CLR_UNUSED_PPN;
            vpn2pt[level2] |= PTE_A;
        } else {
    ...
    //uintptr_t ppn_addr = 0x64800000; // scratchpad
    uintptr_t ppn_addr =   0x7ffff000; // scratchpad    //
    /*    
33 2         2          1 
10 987654321 098765432 109876543210
01 100100100 0xxxxxxxx xxxxxxxxxxxx
   <------------------------------> will be replaced by Virtual address..
    */  
    //
    if (level2 == 0) { // code
    ...
    //
    vpn2pt[level2] = new_pte | PTE_A | PTE_D | PTE_SOFT;
    flush_page(addr);

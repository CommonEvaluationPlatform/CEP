...
// Description:    titleLink BFM master model for UL only
// Notes:          
//
//************************************************************************
   ...
   task checkReset;
	 ...
	 if (reset) begin
	    ...
	    while (reset && !tl_err && (i < MAX_TIMEOUT)) begin
		  ...
		  $display("** timeout while waiting for reset to de-asserted");
		  tl_err = 1;
	       end
	       //
	    end
...
//`define WRITE_PARTIAL   
   task selfTest;
	 ...
	 //
	 adr = 'h700F_0000;
	 if (DEBUG) $display("=== TL-UL Checking FPGA version ===");	 
	 wd64 = 'h0002_0000_0000_0000;
	 for (l=0;l<2;l=l+1) begin // RO
	    tl_a_ul_read(CHIP_ID & 1, adr, rd64); // source=0 or 1 ONLY
	    if (wd64 !== rd64) begin
	       $display("%m: **ERROR** Version Reg adr=0x%x exp=0x%x act=0x%x %t",adr,wd64,rd64,$time);
	       error = 1;
	    end
	    else begin
	       $display("%m: OK Version adr=0x%x exp=0x%x act=0x%x %t",adr,wd64,rd64,$time);	    
	    end
	 ...
	 // Using PUTPARTIAL
	 //
	 $display("%m: === TL-UL Data bus walk 0/1 test with Partial mask ===");	 
	 adr = baseReg;	 
	 for (i=0;i<64;i=i+1) begin
	    ...
	    // write
	    tl_a_ul_write_generic(CHIP_ID & 1, adr, {wd64[63:32],32'h12345678},'hF0); // upper
	    tl_a_ul_write_generic(CHIP_ID & 1, adr, {32'h9ABCDEF0,wd64[31:0]},'h0F); // lower
	    // read back and compare
	    tl_a_ul_read(CHIP_ID & 1, adr, rd64); // source=0 or 1 ONLY
	    //
	    if (wd64 != rd64) begin
	       $display("%m: **ERROR** adr=0x%x exp=0x%x act=0x%x i=%d",adr,wd64,rd64,i);
	       error = 1;
	    end
	    else begin
	       $display("%m: OK adr=0x%x exp=0x%x act=0x%x i=%d",adr,wd64,rd64,i);	    
	    end
	 ...
	 // Using PUTFULL
	 //
	 $display("%m: === TL-UL Data bus walk 0/1 test with full mask ===");	 
	 adr = baseReg;	 
	 for (i=0;i<64;i=i+1) begin
	    if (error) break;	    
	    case (CHIP_ID)
   ...
   task fir_test;
	 ...
	 for (i=0;i<32;i=i+1) begin
	    ...
	    if (rd64[31:0] !== fir_exp[i]) begin
	       $display("%m: ERROR: FIR mismatch act=0x%x != exp=0x%x i=%d",rd64,fir_exp[i],i);
	       repeat (10) @(posedge clock);      	    
	       $finish;	    
	    end
	    else begin
	       $display("%m: OK: FIR compare act=0x%x != exp=0x%x i=%d",rd64,fir_exp[i],i);	       
	    end
	 end
      end
   endtask // for

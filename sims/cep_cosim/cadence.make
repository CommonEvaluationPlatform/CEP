...
#
override VSIM_CMD_LINE = "${XRUN_CMD} -64bit -R -xmlibdirname ${BLD_DIR}/xcelium.d  -afile ${V2C_TAB_FILE} -loadpli1 ${LIB_DIR}/libvpp.so -sv_lib ${LIB_DIR}/libvpp.so -loadvpi ${BLD_DIR}/xcelium.d/run.d/librun.so:boot ${CADENCE_COV_RUN_ARGS} "


${BLD_DIR}/.cadenceBuild : ${VERILOG_DEFINE_LIST} ${SIM_DIR}/common.make ${MKFILE_DIR}/cep_buildChips.make ${DUT_XILINX_TOP_FILE} ${BLD_DIR}/${SIM_DEPEND_TARGET}_OTHER_LIST ${BHV_DIR}/ddr3.v
...
ifeq (${CADENCE},1)
	...
	#(cd ${CAD_TOP_COVERAGE}; ${IMC_CMD}  -execcmd "load -run ${CAD_TOP_COVERAGE}/merge; report_metrics -detail -report_type regular -overwrite -all -out ${CAD_TOP_COVERAGE}/merge/detail_report " ;)
endif

...
trait CanHavePeripheryFIR { this: BaseSubsystem =>
  val firnode = p(PeripheryFIRKey).map { params =>
    ...
    InModuleBody { firmodule.module.reset := coreattachparams.slave_bus.module.reset }
    InModuleBody { firmodule.module.clock := coreattachparams.slave_bus.module.clock }

...
case object PeripheryDFTKey extends Field[Seq[COREParams]](Nil)

// This trait "connects" the core to the Rocket Chip and passes the parameters down
// to the instantiation
trait CanHavePeripheryDFT { this: BaseSubsystem =>
  val dftnode = p(PeripheryDFTKey).map { params =>
    ...
    InModuleBody { dftmodule.module.reset := coreattachparams.slave_bus.module.reset }
    InModuleBody { dftmodule.module.clock := coreattachparams.slave_bus.module.clock }
...
//--------------------------------------------------------------------------------------
class dftTLModule(coreattachparams: COREAttachParams)(implicit p: Parameters) extends LazyModule {
  ...
  lazy val module = new dftTLModuleImp(coreattachparams.coreparams, this)
...
//--------------------------------------------------------------------------------------
class dftTLModuleImp(coreparams: COREParams, outer: dftTLModule) extends LazyModuleImp(outer) {
